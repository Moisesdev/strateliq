...
export default function Register() {
  ...
  const { user, loading: authLoading, register, loginGoogle } = useAuth();
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setFreeRegActive(data.free_registration_active);
      } catch (e) {
        ...
        <div className="w-full max-w-sm">
          {!freeRegActive ? (
            <div className="text-center space-y-6 animate-fade-up">
              <div className="inline-flex h-12 w-12 items-center justify-center rounded-2xl border border-border/60 bg-secondary">
                <ArrowLeft className="h-5 w-5 text-primary rotate-45" strokeWidth={1.5} />
              </div>
              <div>
                <h1 className="font-display text-2xl md:text-3xl font-semibold tracking-tight">Registro cerrado</h1>
                <p className="mt-3 text-muted-foreground text-sm leading-relaxed">
                  El registro de cuentas gratuitas está desactivado temporalmente. Para acceder a la plataforma, debes adquirir una suscripción de pago.
                </p>
              </div>
